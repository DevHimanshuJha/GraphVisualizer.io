...
class MainActivity : AppCompatActivity(), View.OnTouchListener, View.OnDragListener, AdapterView.OnItemSelectedListener {//inhertance baki sab obje inherit unke obj usekr rah
var pointAForLine= PointF(10f,10f)
    var pointBForLine= PointF(10f,10f)//is point  node bana chalu hoga
    ...
    override fun onCreate(savedInstanceState: Bundle?) {//agr phlel se state h toh store in bundlel
        super.onCreate(savedInstanceState)

        with(window){//kotlin funtion that enables th transition and allow animation bw activity
            requestFeature(Window.FEATURE_CONTENT_TRANSITIONS)//req transition
            enterTransition = Explode()//
        }
        //set the screen first wali
        setContentView(R.layout.activity_main)
        // Initialize and set up the graph algorithm spinner
